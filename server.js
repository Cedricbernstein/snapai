...
{"styleAnalysis":"one sentence describing the reference style","prompt":"Write a 150-word ultra-detailed prompt describing: exact background, surface, lighting direction and quality, color palette, props, shadows, atmosphere from the reference images. The product from the input image must appear exactly as-is in the scene, preserving every detail of its shape, label, colors and design. Only the environment changes."}`,
        messages: [{
          role: "user",
          content: [
            ...refParts,
            { type: "image", source: { type: "base64", media_type: prod.mime, data: prod.data } },
            { type: "text", text: `First ${refs.length} image(s) = reference style. Last image = product to place into that style. ${styleHints[style] || styleHints.auto} Output ONLY JSON.` }
          ]
        }]
      })
    });
...
app.post("/api/generate-image", async (req, res) => {
  ...
  try {
    // Use gpt-image-1 generations endpoint (text-to-image with detailed prompt)
