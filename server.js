...
// Step 1: Claude analyzes references and creates prompt
...
{"styleAnalysis":"one sentence describing the reference style","prompt":"Write a 150-word ultra-detailed prompt describing: exact background, surface, lighting, colors, props, shadows, atmosphere from the reference images. End with: Place the product shown in the input image exactly as-is into this scene, preserving every detail of the product."}`,
        messages: [{
          role: "user",
          content: [
            ...refParts,
...
// Step 2: GPT-Image-1 generates the actual image
app.post("/api/generate-image", async (req, res) => {
  ...
  try {
    // Use GPT-Image-1 with image editing (best for product consistency)
    ...
    form.append("image[]", imgBuffer, { filename: "product.jpg", contentType: productImageMime });
    ...
    if (!b64) return res.status(500).json({ error: "Kein Bild von OpenAI" });
